...
```sh
fawltydeps --exclude my_excludes.txt
```

When not given, the `--exclude-from` option will default to reading exclude
patterns from `./.ignore`, `./.gitignore`, and `./.git/info/exclude`.
...
- `exclude_from`: Files (following the .gitignore format) containing exclude
  ...
  and/or Python environments. Defaults to `exclude_from = [".gitignore"]`,
  meaning that exclude patterns from `./.gitignore` will be observed by default.
